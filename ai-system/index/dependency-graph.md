...
> **2026-09-23 query/preview (drift fixed current):** List pages (`assets/users/approvals/audit-logs`) sync filters to URL via `useSearchParams` + `Suspense` + `router.replace` (debounced search); Sidebar filters nav by `preview-role` cookie; middleware overrides `x-user-role` for SUPERADMIN preview; Departments/Roles CRUD via `SystemConfig` category.

### Frontend Hooks

- `approvals/page.tsx:fetchApprovals` — `useCallback` with `[page, pageSize, filters.status, filters.type]` → `useEffect` on `[fetchApprovals]` — URL-synced `?status & ?type` via `useSearchParams` + `Suspense` + `router.replace`, `loading ? TableSkeleton`
- `assets/page.tsx:fetchAssets` — `useCallback` with `[page, pageSize, filters.status, filters.category, filters.search]` — URL-synced `?status & ?category & ?search` + `?assignedToId/?department` passthrough, debounced search 400ms, `loading ? TableSkeleton`
- `audit-logs/page.tsx:fetchLogs` — `useCallback` with `[page, pageSize, filters.entityType, filters.action, filters.actorId]` — URL-synced `?entityType & ?action & ?actorId`, `loading ? TableSkeleton`
- `users/page.tsx:fetchUsers` — `useCallback` with `[page, pageSize, filters.department, filters.role, filters.search]` — URL-synced `?department & ?role & ?search` (from `/admin` dept links like `/users?department=HR`), debounced search, `loading ? TableSkeleton`
...
src/app/api/assets/assign/route.ts (now supports reassignment/transfer: creates AssetTransfer, audit TRANSFER vs ASSIGN)
     ├──→ src/lib/prisma.ts
     ├──→ src/lib/auth.ts
     ├──→ src/lib/audit.ts
     └──→ @prisma/client (Asset, User, Acknowledgement, AssetTransfer)
...
src/middleware.ts (Edge, async)
     ...
     └──→ Reads preview-role cookie; if payload.role===SUPERADMIN and allowed preview, overrides x-user-role with preview-role; sets x-preview-role/x-real-role headers, response x-preview-role
...
src/app/api/admin/departments/route.ts (GET aggregated enum+config, POST/PATCH/DELETE via SystemConfig category=department, audit-logged, non-breaking)
     ├──→ src/lib/prisma.ts
     ├──→ src/lib/auth.ts
     ├──→ src/lib/audit.ts
     └──→ @prisma/client (SystemConfig)

src/app/api/admin/roles/route.ts (GET aggregated enum+config, POST/PATCH/DELETE via SystemConfig category=role; enum edit creates override, enum delete blocked)
     ├──→ src/lib/prisma.ts
     ├──→ src/lib/auth.ts
     ├──→ src/lib/audit.ts
     └──→ @prisma/client (SystemConfig)

src/app/api/admin/preview/route.ts (POST/DELETE/GET preview-role cookie, SUPERADMIN only)
     ├──→ src/lib/prisma.ts
     ├──→ src/lib/auth.ts
     └──→ @prisma/client (User)
...
DashboardLayout (state: collapsed, mobileOpen, localStorage sidebar-collapsed)
     ├──→ Sidebar (props: collapsed, mobileOpen) — drawer + overlay, collapsible, signout, preview banner (filters nav by preview-role cookie, exit preview)
     │       └──→ Link (next/link), Icon (inline SVG), useRouter (logout, preview), useSearchParams-derived nav filter (baseNavigation + roles array)
     │
...
AdminPage (editable, 6 tabs)
     ├──→ Card, CardHeader, CardTitle, CardContent
     ├──→ Button, Badge, Select, Input, Textarea, Skeleton
     ├──→ Tabs, TabsList, TabsTrigger, TabsContent (Overview, Departments, Roles, Accessories, Settings, Preview)
     ├──→ Link
     ├──→ fetch /api/accessories (CRUD) — edit modal, delete
     ├──→ fetch /api/admin/configs (GET/PATCH) — settings save
     ├──→ fetch /api/admin/departments (GET/POST/PATCH/DELETE) — Departments CRUD table, add/edit form, audit-logged
     ├──→ fetch /api/admin/roles (GET/POST/PATCH/DELETE) — Roles CRUD table, add/edit form, enum override, audit-logged
     ├──→ fetch /api/admin/preview (POST/DELETE/GET) — Preview As selector (SUPERADMIN), cookie handling, exit preview
     └──→ Links to /admin/import, /admin/backup, /admin/accessories/new, /users?department=<code>
